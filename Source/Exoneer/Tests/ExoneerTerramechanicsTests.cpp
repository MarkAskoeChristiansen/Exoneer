...
bool FExoneerTerraSinkageTest::RunTest(const FString& Parameters)
{
	...
	const float ZSand = SolveRigidSinkage(Load, Width, Radius, Sand);
	NearRel(*this, TEXT("T1 sand sinkage"), ZSand, 0.067540f);
	NearRel(*this, TEXT("T1 patch length"), ChordPatchLength(ZSand, Radius), 0.189637f);
	// Bekker cross-check: solved z satisfies p = k_eq * z^n at the patch pressure.
	const float Patch = ChordPatchLength(ZSand, Radius);
	const float Pressure = Load / (Width * Patch);
	NearRel(*this, TEXT("T1 Bekker consistency"), KEq(Sand, Width) * FMath::Pow(ZSand, 1.1f), Pressure);

	// T2: clay sinks far less at equal load (low exponent stiffens shallow response).
	const float ZClay = SolveRigidSinkage(Load, Width, Radius, Clay);
	NearRel(*this, TEXT("T2 clay sinkage"), ZClay, 0.026119f);
	TestTrue(TEXT("T2 clay < sand"), ZClay < ZSand);

	// Monotonic in load.
	const float ZHeavy = SolveRigidSinkage(6000.f, Width, Radius, Sand);
	...
	const float ZSpin = SolveRigidSinkage(Load, Width, Radius, Sand, 0.8f);
	...
	const float ZFirm = SolveRigidSinkage(Load, Width, Radius, FirmGroundDefault());
	NearRel(*this, TEXT("firm ground sinkage"), ZFirm, 0.00114544f, 5e-3f);

	// No load, no contact.
	TestEqual(TEXT("W=0 gives z=0"), SolveRigidSinkage(0.f, Width, Radius, Sand), 0.f);
	return true;
}
...
bool FExoneerTerraResistanceTest::RunTest(const FString& Parameters)
{
	...
	const float RcLight = CompactionResistance(Width, 0.067540f, Sand);
	const float RcHeavy = CompactionResistance(Width, 0.106676f, Sand);
	...
	NearRel(*this, TEXT("T5 bulldozing"), BulldozingResistance(Width, 0.067540f, Sand), 66.598f);

	// T9: slip raises total resistance (n_eff feeds Rc; z from the slip-sinkage solve).
	const float RcSpin = CompactionResistance(Width, 0.162636f, Sand, 0.8f);
	const float RbSpin = BulldozingResistance(Width, 0.162636f, Sand);
	...
	TestEqual(TEXT("Rc(0) = 0"), CompactionResistance(Width, 0.f, Sand), 0.f);
	TestEqual(TEXT("Rb(0) = 0"), BulldozingResistance(Width, 0.f, Sand), 0.f);
	return true;
}
...
bool FExoneerTerraTractionTest::RunTest(const FString& Parameters)
{
	...
	const float Area = Width * Patch;
	const float Budget = ShearBudget(Area, Load, Sand);
	...
	const float F05 = CombinedShearForces(Area, Load, 0.05f, 0.f, Patch, Sand).LongitudinalN;
	const float F20 = CombinedShearForces(Area, Load, 0.20f, 0.f, Patch, Sand).LongitudinalN;
	const float F80 = CombinedShearForces(Area, Load, 0.80f, 0.f, Patch, Sand).LongitudinalN;
	...
	TestEqual(TEXT("F(0) = 0"), CombinedShearForces(Area, Load, 0.f, 0.f, Patch, Sand).LongitudinalN, 0.f);
	...
	const FShearForces Combined = CombinedShearForces(Area, Load, 0.20f, FMath::DegreesToRadians(8.f), Patch, Sand);
...
bool FExoneerTerraRegimeTest::RunTest(const FString& Parameters)
{
	...
	const float CriticalPressure = CriticalGroundPressure(Load, Width, Radius, Loam);
	...
	const FWheelContactSolution AtNominal = SolveWheelContact(Load, Width, Radius, 80000.f, 15000.f, Loam, 0.f, true);
	...
	NearRel(*this, TEXT("T8 Rc at 95 kPa"), CompactionResistance(Width, AtNominal.SinkageM, Loam), 208.67f);

	// CTIS drop to 0.6x inflation: sinkage -44 percent, compaction resistance -63 percent.
	const FWheelContactSolution AtLowPressure = SolveWheelContact(Load, Width, Radius, 48000.f, 15000.f, Loam, 0.f, false);
	...
	NearRel(*this, TEXT("T8 CTIS Rc"), CompactionResistance(Width, AtLowPressure.SinkageM, Loam), 76.957f);
	...
	TestFalse(TEXT("hysteresis holds flexible inside the band"),
		SolveWheelContact(Load, Width, Radius, JustBelow, 0.f, Loam, 0.f, false).bRigid);
	TestTrue(TEXT("hysteresis holds rigid inside the band"),
		SolveWheelContact(Load, Width, Radius, CriticalPressure * 0.99f, 0.f, Loam, 0.f, true).bRigid);
	TestTrue(TEXT("clearly above the band goes rigid"),
		SolveWheelContact(Load, Width, Radius, CriticalPressure * 1.05f, 0.f, Loam, 0.f, false).bRigid);

	// No load: contact-free solution, effective radius intact.
	const FWheelContactSolution Airborne = SolveWheelContact(0.f, Width, Radius, 80000.f, 15000.f, Loam, 0.f, true);
	TestEqual(TEXT("airborne sinkage"), Airborne.SinkageM, 0.f);
	TestEqual(TEXT("airborne effective radius"), Airborne.EffectiveRadiusM, Radius);
	return true;
}
